...
def _execution_response(execution: ActionExecution) -> ActionExecutionResponse:
    return ActionExecutionResponse(
        ...
        outcome="failed" if execution.error_code else "succeeded",
...
async def list_action_proposals(
    ...
    responses = [await _to_response(session, user.id, proposal, sources) for proposal in proposals]
...
async def get_action_proposal(
    ...
    return await _to_response(session, user.id, proposal, await _source_map(session, user.id))
...
async def execute_action_proposal(
    ...
    try:
        proposal, _ = await ActionProposalService(session, user.id).execute(proposal_id)
    except ProposalConflictError as exc:
        return _conflict_response(exc)
    return await _to_response(session, user.id, proposal, await _source_map(session, user.id))
