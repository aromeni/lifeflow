...
**Superseded note (Stage 11A Phase 4C, 2026-08-02):** row S11A-P4B-066's "PKCE P2 condition revalidated... kept open and non-blocking" and row S11A-P4B-069's "both pre-existing P2s (PKCE, 7-day constraint)" accurately describe what this task did at the time, but the underlying premise — that the connector-consent flow lacked PKCE — was never true of the code; only its *security implication* was checked against Google's documentation, not the premise itself against `oauth_state.py`. This was found and closed during the Phase 4C integrity check: see `phase-4b/defect-register.md`'s corrected P2 entry and the new `test_connector_connect_redirect_also_carries_pkce` regression test. This note supersedes those two rows' PKCE content without rewriting the historical record of what was verified when.
