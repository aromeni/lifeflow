...
- T11: `state` and a PKCE `code_verifier`/`code_challenge` (S256) are generated per-flow and stored in the existing signed session cookie with a short server-checked TTL, cleared on first use; the connector-consent flow additionally binds `state` to the signed-in `user_id`. Redirect URIs are exact-match against configured settings, never derived from the request `Host` header. Sign-in and connector-consent use separate client configurations (D10), so a redirect-URI or state value from one flow is meaningless in the other. **Stage 11A Phase 4B** (test-account connection readiness, no real account connected) inspected this flow against Google's real-world callback shapes and closed three coverage gaps: denied/cancelled consent (`?error=...`) now clears the pending flow explicitly instead of leaving it until TTL expiry; replay of an already-consumed callback and callback-after-logout are now directly tested at the HTTP route level (both were already safe by construction — only the regression proof was missing). **Stage 11A Phase 4C** corrected a documentation error inherited from Phase 4B: the connector-consent flow was recorded as lacking PKCE, but `begin_oauth_flow` has generated and verified PKCE for both the sign-in and connector-consent flows since Stage 7 — only the OIDC `nonce` is sign-in-only. A new regression test (`test_connector_connect_redirect_also_carries_pkce`) closes the coverage gap; see the Phase 4B defect register for the correction.
