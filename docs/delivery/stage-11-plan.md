...
While recruitment remains blocked, [Stage 11A](stage-11a-owner-validation-plan.md) defines an owner-operated validation programme — synthetic scenario walkthroughs, created but not connected disposable test accounts, a planned (not yet started) 14–30 day soak period, failure/recovery exercises, a security/privacy review, and an owner usability self-review explicitly labelled as engineering evidence, never participant evidence. Phase 4C created the isolated Google environment but kept OAuth consent and account connection blocked. Its purpose is to make sure a future participant is never the first person to find a defect Stage 11A should have already caught. Stage 11A's own exit decision does not authorise recruitment — that remains gated separately by [recruitment-authorisation-checklist.md](../evaluation/stage-11/recruitment-authorisation-checklist.md).
