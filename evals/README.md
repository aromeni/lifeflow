...
- `run_evals.py` — runner; invoke via `./scripts/run-evals.sh [det|det+mock|det+anthropic|brief|brief+mock]`. Brief modes exit non-zero on failure, so they can gate CI.
